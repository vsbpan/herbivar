...
get_dist_test_sim(list(allo_fit,
                       allo_fit,
                       allo_fit),
                  test = c("ks"), 
                  nboot = 2, 
...
apply_probes(
  list("a" = x,"b" = x, "c" = x), 
  add.id = FALSE, 
  probes = c("mean", "Hoover", "J.index", "cd", "cv", "N","max","lac","Gini")
)
...
```{r}
set.seed(12)
bite_size_fit <- fit_bite_size(rallo(100, max.phi = 0.4), 
                               min.phi = 0.005, 
                               family = "all")
bite_size_fit
```
